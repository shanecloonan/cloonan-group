...
export default function GatewayPage() {
  ...
  const loadNetworkInfo = useCallback(async () => {
    ...
    try {
      const stats = await ArweaveGateway.getGatewayStats();
      setGwStats(stats);
    } catch { /* silent */ }
    ...
    for (const gateway of ARWEAVE_DIRECT_GATEWAYS) {
      ...
      try {
        ...
        setGwHealth((prev) => ({ ...prev, [gateway]: "down" }));
      }
    }
  }, [gw]);

  useEffect(() => {
    if (tab === "network") loadNetworkInfo();
  }, [tab, loadNetworkInfo]);

  useEffect(() => {
    if (tab !== "network") return;
    const iv = setInterval(loadNetworkInfo, 30000);
  ...
  const searchExplorer = useCallback(async () => {
    ...
    try {
      if (expType === "txid") {
        ...
        const tags: ArweaveTag[] = (tx.tags ?? []).map((t) => {
          try { return { name: atob(t.name), value: atob(t.value) }; }
          catch { return t; }
        });
        const node = {
          id: tx.id,
          ...
          recipient: tx.target || "",
          owner: { address: "", key: tx.owner || "" },
          fee: { winston: tx.reward || "0", ar: winstonToAr(tx.reward || "0") },
          quantity: { winston: tx.quantity || "0", ar: winstonToAr(tx.quantity || "0") },
          data: { size: tx.data_size || "0", type: null },
          tags,
          block: null,
          parent: null,
        };
        ...
        else if (expType === "tags") {
          try {
            const parsed = JSON.parse(expQuery.trim());
            params.tags = Array.isArray(parsed) ? parsed : [parsed];
          } catch {
            params.tags = [{ name: "Content-Type", values: [expQuery.trim()] }];
          }
        }
  ...
  const computeUploadCost = useCallback(async () => {
    ...
    try {
      const cost = await gw.estimateCost(size);
      setUploadCost(cost);
    } catch { setUploadCost(null); }
  }, [gw, uploadFile, uploadText]);

  useEffect(() => {
    if (tab === "upload") {
      const t = setTimeout(computeUploadCost, 500);
      return () => clearTimeout(t);
    }
  }, [tab, computeUploadCost]);

  const addTag = useCallback(() => {
    if (!uploadNewTagName.trim()) return;
    setUploadTags((prev) => [...prev, { name: uploadNewTagName.trim(), value: uploadNewTagValue.trim() }]);
    setUploadNewTagName("");
    setUploadNewTagValue("");
  ...
  const handleUpload = useCallback(async () => {
    if (!arweaveWallet) {
      setUploadStatus({ msg: "Connect an Arweave wallet first (Wallets page)", type: "error" });
      return;
    }
    const file = uploadFile;
    const text = uploadText.trim();
    if (!file && !text) {
      setUploadStatus({ msg: "Add text or choose a file", type: "error" });
      return;
    }
    setUploadStatus({ msg: "Preparing upload...", type: "loading" });
    setUploadProgress(0);
    try {
      let result;
      if (file) {
        result = await gw.uploadFile(file, uploadTags, arweaveWallet.jwk, uploadDesc || undefined, (pct) => setUploadProgress(pct));
      } else {
        ...
        result = await gw.uploadData(data, tags, arweaveWallet.jwk, uploadDesc || undefined, (pct) => setUploadProgress(pct));
      }
      if (result.status === 200) {
        setUploadStatus({ msg: `Uploaded! TX: ${result.txId}`, type: "success" });
        setUploadText("");
        setUploadFile(null);
        setUploadDesc("");
        setUploadTags([]);
        ...
        setUploadStatus({ msg: `Upload failed (status ${result.status})`, type: "error" });
      }
    } catch (e) {
  ...
  const loadUploads = useCallback(async () => {
    ...
    try {
      const data = await ArweaveGateway.getUploads();
      setUploads(data);
    } catch { /* silent */ }
  ...
  useEffect(() => {
    if (tab === "history" && user) loadUploads();
  }, [tab, user, loadUploads]);

  /* ================================================================ */
  /*  GraphQL tab logic                                                */
  /* ================================================================ */

  const GQL_TEMPLATES: { label: string; query: string }[] = useMemo(() => [
    {
      label: "My Transactions",
      query: arweaveWallet
        ? buildGqlQuery({ owners: [arweaveWallet.address], first: 25, sort: "HEIGHT_DESC" })
        : "# Connect an Arweave wallet first",
    },
    {
      label: "By Content Type",
      query: buildGqlQuery({ tags: [{ name: "Content-Type", values: ["image/png"] }], first: 10, sort: "HEIGHT_DESC" }),
    },
    {
      label: "By App Name",
      query: buildGqlQuery({ tags: [{ name: "App-Name", values: ["MoneyFund"] }], first: 25, sort: "HEIGHT_DESC" }),
    },
    {
      label: "Recent Large Transfers",
      query: buildGqlQuery({ first: 10, sort: "HEIGHT_DESC" }),
    },
  ], [arweaveWallet]);
  ...
  const runGql = useCallback(async () => {
    ...
    setGqlResult("");
    try {
      const result = await gw.rawGraphQL(gqlQuery.trim());
      setGqlResult(JSON.stringify(result, null, 2));
    } catch (e) {
      setGqlResult(`Error: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setGqlLoading(false);
    }
  }, [gw, gqlQuery]);
  ...
  const loadBookmarks = useCallback(async () => {
    try {
      const data = await ArweaveGateway.getBookmarks();
      setBookmarks(data);
    } catch { /* silent */ }
  }, []);

  useEffect(() => {
    if (tab === "bookmarks" && user) loadBookmarks();
  }, [tab, user, loadBookmarks]);
  ...
  const deleteBookmark = useCallback(async (id: string) => {
    await ArweaveGateway.removeBookmark(id);
    loadBookmarks();
  }, [loadBookmarks]);
      ...
      <div className="w-full max-w-[860px] mx-auto space-y-5">
        ...
        <div className={`${card} p-1.5 flex gap-1 overflow-x-auto`}>
          {TABS.map((t) => (
            <button
              ...
              type="button"
              onClick={() => setTab(t.id)}
              className={`flex-1 min-w-0 h-10 rounded-xl text-xs sm:text-sm font-medium transition-all cursor-pointer flex items-center justify-center gap-1 whitespace-nowrap px-2 ${
                tab === t.id
          ...
          <div className="space-y-4">
            ...
            <div className={`${card} p-5`}>
              ...
              {netInfo ? (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div>
                    <p className="text-2xl font-bold text-white tracking-tight">{netInfo.height?.toLocaleString()}</p>
                    <p className="text-[10px] text-white/30 mt-0.5 uppercase">Block Height</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-white tracking-tight">{netInfo.peers}</p>
                    <p className="text-[10px] text-white/30 mt-0.5 uppercase">Peers</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-white tracking-tight">{netInfo.queue_length ?? 0}</p>
                    <p className="text-[10px] text-white/30 mt-0.5 uppercase">Queue</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-white tracking-tight">v{netInfo.version}</p>
                    <p className="text-[10px] text-white/30 mt-0.5 uppercase">Version</p>
                  </div>
                </div>
              ) : (
            ...
            {costPerMb && (
              <div className={`${card} p-5`}>
                ...
                <div className="grid grid-cols-3 gap-4 mt-3">
                  <div>
                    <p className="text-lg font-bold text-white">{parseFloat(costPerMb.ar).toFixed(8)}</p>
                    <p className="text-[10px] text-white/30 uppercase">AR</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-white">${parseFloat(costPerMb.usd).toFixed(4)}</p>
                    <p className="text-[10px] text-white/30 uppercase">USD</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-white">${costPerMb.usd_per_ar?.toFixed(2)}</p>
                    <p className="text-[10px] text-white/30 uppercase">AR/USD Rate</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className={`${card} p-5`}>
              ...
              <div className="mt-3 space-y-2">
                {ARWEAVE_DIRECT_GATEWAYS.map((gw) => {
                  const status = gwHealth[gw] || "checking";
                  return (
                    <div key={gw} className="flex items-center gap-3">
                      <span className={`w-2.5 h-2.5 rounded-full ${
                        status === "up" ? "bg-emerald-400" : status === "down" ? "bg-red-400" : "bg-yellow-400 animate-pulse"
                      }`} />
                      <span className="text-sm text-white/60 font-mono">{gw.replace("https://", "")}</span>
                      <span className={`ml-auto text-[10px] uppercase font-semibold ${
                        status === "up" ? "text-emerald-400" : status === "down" ? "text-red-400" : "text-yellow-400"
                      }`}>
                        {status}
                      </span>
                    </div>
            ...
            {gwStats && gwStats.totalRequests > 0 && (
              <div className={`${card} p-5`}>
                ...
                <div className="grid grid-cols-3 gap-4 mt-3">
                  <div>
                    <p className="text-2xl font-bold text-white">{gwStats.totalRequests}</p>
                    <p className="text-[10px] text-white/30 uppercase">Requests</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-white">{(gwStats.cacheHitRate * 100).toFixed(1)}%</p>
                    <p className="text-[10px] text-white/30 uppercase">Cache Hit</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-white">{gwStats.avgResponseMs}ms</p>
        ...
        {tab === "explorer" && (
          <div className="space-y-4">
            <div className={`${card} p-5 space-y-3`}>
              <div className="flex flex-col sm:flex-row gap-2">
                <select
                  ...
                  className={`sm:w-40 ${inputCls} appearance-none cursor-pointer`}
                >
                ...
                <input
                  ...
                  placeholder={expType === "txid" ? "Transaction ID..." : expType === "tags" ? '{"name":"Content-Type","values":["image/png"]}' : "Arweave address..."}
                        ...
                        <div className="px-4 pb-4 space-y-3 border-t border-white/[0.04]">
                          <div className="grid grid-cols-2 gap-3 pt-3">
                            <div>
                              ...
                              <p className="text-xs font-mono text-white/50 break-all">{n.owner?.address || shorten(n.owner?.key || "", 20)}</p>
                            </div>
                            <div>
                              <span className="text-[10px] text-white/30 uppercase">Recipient</span>
                              <p className="text-xs font-mono text-white/50 break-all">{n.recipient || "—"}</p>
                            </div>
                            <div>
                              <span className="text-[10px] text-white/30 uppercase">Fee</span>
                              <p className="text-xs text-white/50">{n.fee?.ar} AR</p>
                            </div>
                            <div>
                              <span className="text-[10px] text-white/30 uppercase">Quantity</span>
                              <p className="text-xs text-white/50">{n.quantity?.ar} AR</p>
                            </div>
                          </div>
                          {n.tags && n.tags.length > 0 && (
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1.5 mt-1">
                                {n.tags.map((t, i) => (
                                  <span key={i} className="text-[10px] px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/[0.08] text-white/40">
                                    {t.name}: {t.value}
                                  </span>
                                ))}
                              </div>
            ...
            {!expLoading && expResults.length === 0 && expQuery && (
        ...
        {tab === "browser" && (
          <div className="space-y-4">
            <div className={`${card} p-4 flex gap-2`}>
              <input
                value={browseTxId}
                onChange={(e) => setBrowseTxId(e.target.value)}
                placeholder="Enter Transaction ID to view..."
                className={`flex-1 ${inputCls}`}
                onKeyDown={(e) => e.key === "Enter" && loadBrowseData()}
              />
            ...
            {browseData && (
              <div className={`${card} p-5 space-y-3`}>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-300/60 border border-purple-500/20">
                    {browseData.contentType}
                  </span>
                ...
                <div className="rounded-xl overflow-hidden border border-white/[0.06] bg-black/30 min-h-[200px] max-h-[500px] overflow-auto">
                  ...
                  {ArweaveGateway.contentCategory(browseData.contentType) === "video" && (
                    <video src={browseData.url} controls className="max-w-full mx-auto" />
                  )}
                  {ArweaveGateway.contentCategory(browseData.contentType) === "audio" && (
                    <div className="p-8 flex justify-center"><audio src={browseData.url} controls /></div>
                  )}
                  {ArweaveGateway.contentCategory(browseData.contentType) === "html" && (
                    <iframe src={browseData.url} title="Arweave HTML" sandbox="allow-scripts" className="w-full h-[400px] border-0" />
                  )}
                  {(ArweaveGateway.contentCategory(browseData.contentType) === "text" ||
                    ArweaveGateway.contentCategory(browseData.contentType) === "json") && (
                    <BlobTextViewer url={browseData.url} />
                  )}
                  {ArweaveGateway.contentCategory(browseData.contentType) === "pdf" && (
                    <iframe src={browseData.url} title="PDF" className="w-full h-[500px] border-0" />
                  )}
                  {ArweaveGateway.contentCategory(browseData.contentType) === "binary" && (
                    <div className="p-8 text-center text-white/30 text-sm">Binary content. <a href={browseData.url} target="_blank" rel="noopener noreferrer" className="text-purple-400 underline">Download</a></div>
                  )}
                </div>
              </div>
            )}
            ...
            {browseHistory.length > 0 && (
              <div className={`${card} p-4`}>
                ...
                <div className="mt-2 space-y-1">
                  {browseHistory.map((id) => (
                    <button
                      key={id}
                      type="button"
                      onClick={() => { setBrowseTxId(id); loadBrowseData(id); }}
                      className="block w-full text-left text-xs font-mono text-purple-300/50 hover:text-purple-300 py-1 px-2 rounded-lg hover:bg-white/[0.03] transition-colors cursor-pointer truncate"
                    >
                      {id}
                    </button>
                  ))}
          ...
          <div className="space-y-4">
            {!arweaveWallet && (
              <div className={`${card} p-5 text-center`}>
                ...
                <Link href="/wallets" className={`${btnPrimary} inline-flex items-center gap-2`}>Go to Wallets</Link>
              </div>
            )}
            ...
            {arweaveWallet && (
              <>
                <div className={`${card} p-5 space-y-4`}>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Enter text to store permanently..."
                  ...
                  <div>
                    ...
                    <input value={uploadDesc} onChange={(e) => setUploadDesc(e.target.value)} placeholder="What is this upload?" className={inputCls} />
                  </div>
                </div>
                ...
                <button type="button" onClick={handleUpload} disabled={uploadProgress !== null} className={`w-full ${btnPrimary}`}>
                  {uploadProgress !== null ? "Uploading..." : "Upload to Arweave"}
                </button>

                {uploadStatus && (
                  <div className={`${card} p-3 text-xs font-medium ${
          ...
          <div className="space-y-4">
            ...
            {!uploadsLoading && uploads.length === 0 && (
              <div className={`${card} p-10 text-center`}>
                ...
                <p className="text-xs text-white/20 mt-1">Upload data from the Upload tab to see it here.</p>
              </div>
            )}
                ...
                <div className={`${card} p-4`}>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <p className="text-[10px] text-white/30 uppercase">Uploads</p>
                    </div>
                    <div>
                      <p className="text-xl font-bold text-white">{formatBytes(uploads.reduce((s, u) => s + (u.data_size || 0), 0))}</p>
                      <p className="text-[10px] text-white/30 uppercase">Total Data</p>
                    </div>
                    <div>
                      <p className="text-xl font-bold text-white">{uploads.reduce((s, u) => s + parseFloat(u.cost_ar || "0"), 0).toFixed(6)}</p>
                      <p className="text-[10px] text-white/30 uppercase">Total AR</p>
                    </div>
                  </div>
                </div>
                  ...
                  {uploads.map((u) => (
                    <div key={u.id} className={`${card} px-4 py-3 flex items-center gap-3`}>
                      ...
                      <span className={`text-[9px] px-2 py-0.5 rounded-full font-semibold uppercase ${
                        u.status === "confirmed" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" :
                        u.status === "submitted" ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" :
                        u.status === "failed" ? "bg-red-500/10 text-red-400 border border-red-500/20" :
                        "bg-yellow-500/10 text-yellow-400 border border-yellow-500/20"
                      }`}>
                        {u.status}
                      </span>
                      {u.tx_id && (
                        <button type="button" onClick={() => { setBrowseTxId(u.tx_id!); setTab("browser"); loadBrowseData(u.tx_id!); }} className={pillBtn}>View</button>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-4">
            <div className={`${card} p-4`}>
              ...
              <div className="flex flex-wrap gap-2">
                {GQL_TEMPLATES.map((t) => (
                  <button key={t.label} type="button" onClick={() => setGqlQuery(t.query)} className={pillBtn}>{t.label}</button>
                ))}
              </div>
            </div>
            ...
            <div className={`${card} p-5 space-y-3`}>
              ...
              <textarea
                ...
                placeholder="{ transactions(first: 10) { edges { node { id } } } }"
                className={`${inputCls} h-auto py-3 font-mono text-xs`}
                style={{ resize: "vertical" }}
              />
              <button type="button" onClick={runGql} disabled={gqlLoading} className={`w-full ${btnPrimary}`}>
                {gqlLoading ? <span className="w-4 h-4 border-2 border-white/80 border-t-transparent rounded-full animate-spin mx-auto" /> : "Run Query"}
              </button>
            </div>

            {gqlResult && (
              <div className={`${card} p-4`}>
                <span className="text-xs font-medium text-white/30 uppercase tracking-wider mb-2 block">Result</span>
                <pre className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] text-xs text-white/40 max-h-[400px] overflow-auto whitespace-pre-wrap break-all font-mono" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.06) transparent" }}>
                  {gqlResult}
                </pre>
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            {bookmarks.length === 0 && (
              <div className={`${card} p-10 text-center`}>
                ...
                <p className="text-xs text-white/20 mt-1">Save transactions, addresses, or content for quick access.</p>
              </div>
            )}
              ...
              <div className="space-y-1.5">
                {bookmarks.map((bm) => (
                  <div key={bm.id} className={`${card} px-4 py-3 flex items-center gap-3`}>
                    <span className={`text-[9px] px-2 py-0.5 rounded-full font-semibold uppercase ${
                      bm.bookmark_type === "transaction" ? "bg-purple-500/10 text-purple-300 border border-purple-500/20" :
                      bm.bookmark_type === "address" ? "bg-blue-500/10 text-blue-300 border border-blue-500/20" :
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      {bm.bookmark_type === "transaction" && (
                        <button type="button" onClick={() => { setBrowseTxId(bm.target_id); setTab("browser"); loadBrowseData(bm.target_id); }} className={pillBtn}>View</button>
                      )}
                      {bm.bookmark_type === "address" && (
                        <button type="button" onClick={() => { setExpQuery(bm.target_id); setExpType("owner"); setTab("explorer"); }} className={pillBtn}>Explore</button>
                      )}
