  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`${card} p-1.5 flex gap-1`}>
        {TABS.map((t) => (
          <button key={t.id} type="button" onClick={() => { setTab(t.id); setViewItem(null); setFeedViewItem(null); }} className={`flex-1 h-10 rounded-xl text-sm font-medium transition-all cursor-pointer flex items-center justify-center gap-1.5 ${tab === t.id ? "bg-sky-500/20 text-sky-300 shadow-[inset_0_1px_0_rgba(56,189,248,0.2)]" : "text-white/40 hover:text-white/60 hover:bg-white/[0.03]"}`}>
            <span className="text-xs opacity-60">{t.icon}</span>{t.label}
          </button>
        ))}
      </div>
      ...
      {tab === "upload" && (
        <div className="space-y-4">
          <div className={`${card} p-1.5 flex gap-1`}>
            <button type="button" onClick={() => setUploadMode("private")} className={`flex-1 h-10 rounded-xl text-sm font-medium transition-all cursor-pointer flex items-center justify-center gap-2 ${uploadMode === "private" ? "bg-sky-500/15 text-sky-300 shadow-[inset_0_1px_0_rgba(56,189,248,0.2)]" : "text-white/40 hover:text-white/60 hover:bg-white/[0.03]"}`}>
              <span className="w-2 h-2 rounded-full bg-sky-400" style={{ opacity: uploadMode === "private" ? 1 : 0.3 }} />Private
            </button>
            <button type="button" onClick={() => setUploadMode("permawrite")} className={`flex-1 h-10 rounded-xl text-sm font-medium transition-all cursor-pointer flex items-center justify-center gap-2 ${uploadMode === "permawrite" ? "bg-purple-500/15 text-purple-300 shadow-[inset_0_1px_0_rgba(168,85,247,0.2)]" : "text-white/40 hover:text-white/60 hover:bg-white/[0.03]"}`}>
              <span className="w-2 h-2 rounded-full bg-purple-400" style={{ opacity: uploadMode === "permawrite" ? 1 : 0.3 }} />PermaWrite
            </button>
          </div>
          ...
          <button type="button" onClick={handleUpload} disabled={uploadProgress !== null} className={`w-full ${uploadMode === "permawrite" ? btnPermawrite : btnPrimary}`}>
...
function ItemCard({ item, onClick, showBadge = true }: { item: PermawriteItem; onClick: () => void; showBadge?: boolean }) {
  ...
  const thumbUrl = item.visibility === "permawrite" && item.arweave_tx_id ? getArweaveContentUrl(item.arweave_tx_id) : null;
