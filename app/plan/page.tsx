...
const SECTIONS: Section[] = [
  {
    ...
    items: [
      ...
      { title: "Auth gate missing on some contract pages", detail: "Auction, multiswap, and storefront show the wallet selector UI without checking authentication state. They should show AuthPanel when !user || !vaultUnlocked, matching the pattern used by ETF and dividends.", files: "auction-app.tsx, multiswap-app.tsx, storefront-app.tsx", priority: "high" },
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Navbar doesn't show active page", detail: "The navbar links don't indicate which page the user is currently on. Adding an active state (highlighted link, underline, or color change) would improve navigation clarity.", files: "navbar.tsx", priority: "medium" },
      { title: "Accessibility: missing ARIA attributes", detail: "The FAQ accordion lacks aria-expanded and aria-controls. Several form inputs lack proper label associations (htmlFor). The mobile menu has no focus trap. These are WCAG AA compliance gaps.", files: "about-app.tsx, multiple forms, navbar.tsx", priority: "medium" },
      ...
      { title: "Mobile menu doesn't close on browser navigation", detail: "The navbar's open state is only set to false via onClick on nav items. If the user navigates via browser back/forward buttons, the mobile menu stays open, covering the content. Fix: add a useEffect that watches pathname (from usePathname) and calls setOpen(false) when it changes.", files: "navbar.tsx", priority: "medium" },
      { title: "No password strength feedback or confirmation on signup", detail: "During signup, the password also becomes the vault encryption key. There's no minimum length enforcement, no strength indicator, and no confirmation field. A weak password (e.g., '1') means weak wallet encryption. Fix: add minimum length check (8+ chars), a confirmation field, and ideally a strength indicator.", files: "auth-panel.tsx", priority: "medium" },
      ...
      { title: "Hamburger button missing aria-expanded attribute", detail: "The mobile menu toggle button has aria-label='Toggle menu' but no aria-expanded={open}. Screen readers can't determine whether the menu is currently open or closed. Fix: add aria-expanded={open} to the button.", files: "navbar.tsx", priority: "low" },
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Contract addresses on /contracts differ from addresses in ABIs", detail: "Five launcher addresses displayed on the contracts page don't match the addresses actually imported and used by the corresponding page components: Dividends Launcher (0xdf1e... on page vs 0x5ef0... in abis), DAO Launcher (0x8ef4... vs 0xc346...), Multiswap Launcher (0xe01f... vs 0x40af...), Ad-space Launcher (0x346a... vs 0xE01F...), Storefront Launcher (0x20c8... vs 0x15a3...). Users see one address on /contracts but the app interacts with a different one.", files: "contracts/page.tsx, dividends/abis.ts, dao/abis.ts, multiswap/abis.ts, auction/abis.ts, storefront/abis.ts", priority: "high" },
      ...
      { title: "Possibly unused dependencies in package.json", detail: "exceljs and html-to-image are listed in dependencies but may not be imported anywhere in the current codebase (they were likely used by the now-deleted tools pages). If confirmed unused, they add unnecessary bundle weight. Fix: check imports and remove if unused.", files: "package.json", priority: "low" },
